...
import com.optimisticbyte.expensetracker.ui.charts.ChartData
import com.optimisticbyte.expensetracker.ui.charts.TrendPoint
import javax.inject.Inject

@HiltViewModel
class TransactionViewModel @Inject constructor(
    private val repository: TransactionRepository,
    private val budgetManager: com.optimisticbyte.expensetracker.utils.BudgetManager,
    private val backupManager: com.optimisticbyte.expensetracker.utils.BackupManager,
    private val notificationHelper: com.optimisticbyte.expensetracker.utils.NotificationHelper
) : ViewModel() {
    ...
    private val _currentTheme = MutableStateFlow(com.optimisticbyte.expensetracker.ui.theme.AppTheme.System)
    val currentTheme: StateFlow<com.optimisticbyte.expensetracker.ui.theme.AppTheme> = _currentTheme.asStateFlow()
    ...
    fun setTheme(theme: com.optimisticbyte.expensetracker.ui.theme.AppTheme) {
    ...
    fun importFromSms(messages: List<com.optimisticbyte.expensetracker.data.SmsMessage>) {
        viewModelScope.launch {
            val transactions = messages.mapNotNull { msg ->
                // Reuse existing parser
                com.optimisticbyte.expensetracker.core.SmsParser.parseSms(msg.sender, msg.body, msg.date)
            }
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    val pieChartData: StateFlow<List<com.optimisticbyte.expensetracker.ui.charts.ChartData>> = combine(
        transactions,
        categories,
        _filterCategoryId
    ) { txns, cats, filterId ->
        ...
        .map { (txns, cats, filterId) ->
            ...
            val result = top5.mapIndexed { index, (label, value) ->
                com.optimisticbyte.expensetracker.ui.charts.ChartData(
                    label = label,
                    value = value,
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    val trendLineData: StateFlow<List<com.optimisticbyte.expensetracker.ui.charts.TrendPoint>> = transactions
        .map { txns ->
            if (txns.isEmpty()) return@map emptyList()
            ...
            grouped.map { (date, list) ->
                com.optimisticbyte.expensetracker.ui.charts.TrendPoint(
                    dateMillis = date,
